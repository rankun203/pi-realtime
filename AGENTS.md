...
- Voice is a thin relay: `post_message(message)` queues intentional user speech, including follow-ups, with server-owned `user` origin. It cannot independently inspect status/history or invent work. Lifecycle notifications use `nextTurn` without triggering a turn. Device disconnect/restart must never interrupt Pi work.
- Native input turns require a routing tool and use text output; only explicitly scheduled Pi readbacks produce substantive speech. Readbacks are tool-free, out-of-band responses containing the new Pi text and a bounded latest-user-message reference for explicit dictation requests, not the user conversation. Describe code, commands, and long paths by purpose rather than dictating them by default; preserve facts and work status, and leave visible Pi text unchanged. This avoids interpreting an outstanding user question instead of reading Pi's update. `wait_for_user` silently ends overheard/unclear turns; intent recognition remains model-dependent.
- Voice-side history and handover remain private, bounded, and scoped to the attached Pi session/current branch; exclude private reasoning and raw tool results. Original Pi messages remain unchanged. Route follow-up questions to Pi, even when their answer appears in this reference context.
