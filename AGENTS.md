...
- Native input turns require a routing tool and use text output; only explicitly scheduled Pi readbacks produce substantive speech. Readbacks are tool-free, out-of-band responses containing the new Pi text, not the user conversation. This avoids interpreting an outstanding user question instead of reading Pi's update. `wait_for_user` silently ends overheard/unclear turns; intent recognition remains model-dependent.
