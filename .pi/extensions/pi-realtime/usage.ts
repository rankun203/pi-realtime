...
// USD list estimates. 2.1 and 2.1-mini verified against Azure Retail Prices API,
...
export function estimateUsageCost(input: {
	...
	if (!pricing) return 0;
	return (
		dollars(input.input.textTokens, input.input.cachedTextTokens, pricing.text) +
...
export function costExcludedReason(input: {
	provider: ProviderKind;
	model: string;
	source: UsageSource;
}): string | undefined {
	if (input.source === "input_transcription")
		return "Input transcription uses a separate transcription model/rate card; token usage is tracked but excluded from this estimate.";
...
export function aggregateUsage(
	...
	providerSessionId?: ProviderSessionId,
	resets: readonly UsageReset[] = [],
): UsageSummary {
	const resetAt = latestResetAt(resets, providerSessionId);
	const rows = observations.filter(
		(item) => (!providerSessionId || item.providerSessionId === providerSessionId) && item.at > resetAt,
	);
	...
	for (const row of rows) {
		...
		const newlyPriced =
			row.costExcludedReason?.startsWith("No local pricing table for model ") && !costExcludedReason(row);
...
function latestResetAt(resets: readonly UsageReset[], providerSessionId?: ProviderSessionId): number {
	return resets.reduce((latest, reset) => {
		const applies = providerSessionId
			? !reset.providerSessionId || reset.providerSessionId === providerSessionId
			: !reset.providerSessionId;
...
export function renderUsageSummary(summary: UsageSummary, details = false): string {
	if (summary.observations === 0)
		return `realtime usage${summary.providerSessionId ? ` for ${summary.providerSessionId}` : ""}: no usage events observed yet`;
	const lines = [
	...
	if (details)
		lines.push(
			"Note: OpenAI dashboard billing is authoritative; this local estimate uses checked-in pricing constants and excludes separately billed transcription when pricing is unknown.",
		);
	return lines.join("\n");
}
