...
import {
	...
	type RealtimeConfig,
	type RealtimeConfigPatch,
	type RealtimeEvent,
	type RealtimeHistorySummary,
	type RealtimeState,
	...
	type VoiceToolResultRecord,
} from "./types";

export const defaultConfig: RealtimeConfig = {
	primaryProviderSessionId: null,
...
export function createInitialState(config: RealtimeConfig = defaultConfig): RealtimeState {
	return {
		config: { ...config },
		sessions: new Map(),
		primaryProviderSessionId: config.primaryProviderSessionId,
...
export function applyEvent(state: RealtimeState, event: RealtimeEvent): void {
	...
	switch (event.kind) {
		...
		case "session_stopped":
			return applySessionStopped(state, event);
		case "session_primary_changed":
			return applyPrimaryChanged(state, event.providerSessionId);
		case "provider_event":
			return applyProviderEvent(state, event);
		case "context_packet_sent":
			return applyContextPacket(state, event);
		case "voice_tool_call_received":
			return void (
				event.call.status === "pending" && state.pendingToolCalls.set(event.call.voiceToolCallId, { ...event.call })
			);
		case "voice_tool_result_sent":
			return void state.pendingToolCalls.delete(event.result.voiceToolCallId);
		case "voice_instruction_submitted":
			return void (state.lastInstruction = { ...event.instruction });
		case "usage_observed":
			return void state.usage.push({ ...event.observation });
		case "usage_reset":
			return void state.usageResets.push({ at: event.at, providerSessionId: event.providerSessionId });
		case "citation_deck_observed":
			return applyCitationDeck(state, event.deck);
		case "config_changed":
			return applyConfigPatch(state, event.patch);
	}
}

function applySessionStarted(state: RealtimeState, event: Extract<RealtimeEvent, { kind: "session_started" }>): void {
	state.sessions.set(event.session.providerSessionId, {
		...event.session,
		interactionMode: event.session.interactionMode ?? "agent",
	});
...
function applyProviderEvent(state: RealtimeState, event: Extract<RealtimeEvent, { kind: "provider_event" }>): void {
	...
	if (!existing) return;
	const status =
		event.providerEvent.type === "error"
			? "error"
			: event.providerEvent.type === "disconnected"
				? "stopped"
				: event.providerEvent.type === "connected"
					? "active"
					: existing.status;
	state.sessions.set(existing.providerSessionId, {
		...existing,
		status,
		lastProviderEventAt: event.at,
		lastError: event.providerEvent.type === "error" ? event.providerEvent.message : existing.lastError,
	});
}

function applyContextPacket(
	state: RealtimeState,
	event: Extract<RealtimeEvent, { kind: "context_packet_sent" }>,
): void {
...
function applyCitationDeck(state: RealtimeState, deck: CitationDeckSummary): void {
	const active = deck.citationIds.map((citationId, index) => ({
		displayRef: `[${index + 1}]`,
		alias: `@p${index + 1}`,
		citationId,
...
function applyConfigPatch(state: RealtimeState, patch: RealtimeConfigPatch): void {
	...
	const migratedOpenAI =
		typeof openaiWebRTCEnabled === "boolean"
			? {
					openai: {
						...state.config.providerPreferences.openai,
						autoMediaMode: openaiWebRTCEnabled ? ("webrtc" as const) : ("none" as const),
					},
				}
			: {};
	state.config = {
		...state.config,
		...rest,
		defaultInteractionMode: rest.defaultInteractionMode ?? state.config.defaultInteractionMode ?? "agent",
		providerPreferences: { ...state.config.providerPreferences, ...providerPreferences, ...migratedOpenAI },
	};
	state.primaryProviderSessionId = state.config.primaryProviderSessionId;
}

export function sessionStarted(
	session: Omit<VoiceSessionRecord, "status" | "startedAt" | "interactionMode"> &
		Partial<Pick<VoiceSessionRecord, "status" | "startedAt" | "interactionMode">>,
	at = Date.now(),
): RealtimeEvent {
	return {
		...
		session: {
			...session,
			interactionMode: session.interactionMode ?? "agent",
...
export function contextPacketSent(
...
export function voiceInstructionSubmitted(
	...
	return {
		...
		eventId: id("evt"),
		at,
		instruction: { ...input, submittedAt: at, delivery: receipt.delivery },
		receipt,
	};
}
...
export function isRealtimeEvent(value: unknown): value is RealtimeEvent {
	...
	return [
		...
		"provider_event",
		"context_packet_sent",
		"voice_tool_call_received",
		"voice_tool_result_sent",
		"voice_instruction_submitted",
		"usage_observed",
		"usage_reset",
		"citation_deck_observed",
		"config_changed",
	].includes(value.kind);
}
...
export function summarizeDeck(deck: CitationDeck): CitationDeckSummary {
	return {
		...
		count: deck.active.length,
		citationIds: deck.active.map((item) => item.citationId),
		observedAt: deck.observedAt,
	};
}
...
function firstActiveSessionId(state: RealtimeState): ProviderSessionId | null {
	for (const session of state.sessions.values())
		if (session.status === "active" || session.status === "starting") return session.providerSessionId;
	return null;
}
	...
	const providerSessionId =
		"providerSessionId" in event
			...
			: event.kind === "session_started"
				...
				: event.kind === "usage_observed"
					? event.observation.providerSessionId
					: undefined;
	return providerSessionId
		? { kind: event.kind, eventId: event.eventId, at: event.at, providerSessionId }
		: { kind: event.kind, eventId: event.eventId, at: event.at };
}
