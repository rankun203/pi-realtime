...
export type PiSnapshot = { sessionId: string; branchId: string; project: string; busy: boolean; messages: VisibleMessage[] };
...
export type VoiceTransport = {
 connect(input: { sdp: string; model: string; instructions: string; tools: VoiceEvent[]; onEvent(event: VoiceEvent): void; onClose(): void }): Promise<VoiceConnection>;
};
export type VoiceMemory = { version: 1; sessionId: string; branchId: string; summary: string; turns: VisibleMessage[]; observedIds: string[] };
