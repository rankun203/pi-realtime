	...
	return {
		async connect(input) {
			...
			if (config.authMode === "api-key") {
				...
				const secretResponse = await fetch(`${config.baseURL}/realtime/client_secrets`, {
					method: "POST",
			...
			if (!callId || !/^[a-zA-Z0-9_-]+$/.test(callId))
				throw new Error("Provider did not return a valid WebRTC call identifier");
			const hangup = async () => {
				const result = await fetch(`${config.callsUrl}/${encodeURIComponent(callId)}/hangup`, {
					method: "POST",
					headers,
					signal: AbortSignal.timeout(10000),
				});
				if (!result.ok && ![404, 410].includes(result.status))
					throw new Error(`Provider hangup failed (${result.status})`);
			};
