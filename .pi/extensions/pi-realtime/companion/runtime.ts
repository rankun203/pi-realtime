...
export class VoiceCompanion {
 ...
 constructor(private readonly options: CompanionOptions) {
  ...
  this.path = join(options.stateDirectory, `${createHash("sha256").update(this.snapshot.sessionId).digest("hex")}.json`);
  this.memory = this.emptyMemory(this.snapshot);
  try {
   const data = JSON.parse(readFileSync(this.path, "utf8"));
   if (data.version === 1 && data.sessionId === this.snapshot.sessionId && data.branchId === this.snapshot.branchId && typeof data.summary === "string" && data.summary.length <= 4000 && Array.isArray(data.turns) && Array.isArray(data.observedIds)) {
    this.memory = { ...this.memory, summary: data.summary, turns: data.turns.filter((m: any) => typeof m.id === "string" && ["user", "assistant"].includes(m.role) && typeof m.text === "string").slice(-24), observedIds: data.observedIds.filter((id: unknown) => typeof id === "string").slice(-200) };
   }
  } catch { /* First use or an invalid handover: rebuild from authoritative Pi state. */ }
  this.timer = setInterval(() => { void this.tick().catch(() => this.detach()).catch(() => {}); }, 1000);
  this.timer.unref();
 }
 private emptyMemory(pi: PiSnapshot): VoiceMemory { return { version: 1, sessionId: pi.sessionId, branchId: pi.branchId, summary: "", turns: [], observedIds: [] }; }
 private save(): void {
  const temp = `${this.path}.${randomUUID()}.tmp`;
  writeFileSync(temp, JSON.stringify(this.memory), { mode: 0o600, flag: "wx" });
  renameSync(temp, this.path);
 }
 status() { return { connected: !!this.lease, restart: !!this.lease?.restart && ((!this.lease.responding && !this.lease.playing && !this.lease.speaking && !this.lease.awaitingNative) || this.now() - this.lease.startedAt > 59 * 60000), model: this.options.model }; }
 messages() { return this.memory.turns; }
 async connect(sdp: string, takeover = false): Promise<{ answer: string; lease: string }> {
  const operation = this.operations.then(async () => {
   ...
   if (this.lease && !takeover) throw Object.assign(new Error("Another device owns this voice conversation. Explicit takeover required."), { statusCode: 409 });
   await this.detach();
   await this.observe();
   if (this.closed) throw new Error("Pi session changed; open its own voice companion");
   const lease: Lease = { token: randomUUID(), seenAt: this.now(), startedAt: this.now(), speaking: false, responding: false, playing: false, awaitingNative: false, pending: false, restart: false, noticeSent: false, toolIds: new Set(), toolsPending: 0 };
   this.lease = lease;
   try {
    const connection = await this.options.transport.connect({ sdp, model: this.options.model, instructions: companionInstructions(), tools: companionTools(), onEvent: event => this.onEvent(lease, event), onClose: () => { if (this.lease === lease) void this.detach(); } });
    if (this.lease !== lease || this.closed) { await connection.close(); throw new Error("Voice connection superseded"); }
    lease.connection = connection;
    this.send({ type: "conversation.item.create", item: { type: "message", role: "system", content: [{ type: "input_text", text: startupContext(this.snapshot, this.memory) }] } });
    this.memory.observedIds = this.snapshot.messages.map(m => m.id).slice(-200);
    this.save();
    this.options.bridge.lifecycle("Voice device connected. Continue normal work; voice observes your ordinary visible output. No acknowledgement or special speech tools are needed.");
    return { answer: connection.answer, lease: lease.token };
   } catch (error) { if (this.lease === lease) await this.detach(); throw error; }
  });
  this.operations = operation.catch(() => {});
  return operation;
 }
 heartbeat(token: string) {
  if (!this.lease || this.lease.token !== token) throw Object.assign(new Error("Device lease expired or taken over"), { statusCode: 409 });
  this.lease.seenAt = this.now();
  return this.status();
 }
 async release(token: string): Promise<void> { if (this.lease?.token === token) await this.detach(); }
 async detach(): Promise<void> {
  ...
  try { this.save(); }
   ...
   try { await lease.connection?.close(); }
   finally { this.options.bridge.lifecycle("Voice device disconnected. Continue normal work uninterrupted. No acknowledgement needed."); }
  }
 }
 async close(): Promise<void> { this.closed = true; clearInterval(this.timer); await this.detach(); }
 async tick(): Promise<void> {
  await this.observe();
  const lease = this.lease;
  if (!lease) return;
  if (this.now() - lease.seenAt > 45000) { await this.detach(); return; }
  const age = this.now() - lease.startedAt;
  if (age > 58 * 60000) { lease.restart = true; return; }
 ...
 private async observe(): Promise<void> {
  ...
  if (pi.sessionId !== this.memory.sessionId) { await this.close(); return; }
 ...
 private requestRestart(): void {
  ...
  this.send({ type: "conversation.item.create", item: { type: "message", role: "system", content: [{ type: "input_text", text: JSON.stringify({ type: "service_notice", action: "restart", text: "Prepare a fresh voice session. Briefly tell the user you need to reconnect, then invoke restart_voice with a concise handover." }) }] } });
  lease.pending = true;
 }
 private send(event: VoiceEvent): void { this.lease?.connection?.send(event); }
 private flush(): void {
  const lease = this.lease;
  if (!lease?.connection || !lease.pending || lease.responding || lease.speaking || lease.awaitingNative || lease.playing || lease.restart || lease.toolsPending) return;
  lease.pending = false; lease.responding = true;
 ...
 private onEvent(lease: Lease, event: VoiceEvent): void {
  ...
  if (event.type === "input_audio_buffer.speech_started") { lease.speaking = true; lease.awaitingNative = true; }
  ...
  if (event.type === "response.output_audio_transcript.done" || event.type === "conversation.item.input_audio_transcription.completed") {
   ...
   if (typeof event.transcript === "string" && !this.memory.turns.some(m => m.id === id)) {
    this.memory.turns.push({ id, role: event.type.startsWith("response.") ? "assistant" : "user", text: event.transcript.slice(0, 12000), at: this.now(), source: "Voice" });
    this.memory.turns = this.memory.turns.slice(-24); this.save();
   }
  }
  if (event.type === "response.function_call_arguments.done" && typeof event.call_id === "string" && !lease.toolIds.has(event.call_id)) {
   lease.toolIds.add(event.call_id); lease.toolsPending++;
   void this.runTool(lease, event).finally(() => { lease.toolsPending--; });
  }
 }
