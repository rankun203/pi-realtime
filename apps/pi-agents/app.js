...
function renderAgentList() {
 ...
 const list = document.getElementById('agent-list'); list.replaceChildren();
 ...
 for (const agent of agents) {
  ...
  const button = document.createElement('button'); button.className = 'agent-card';
  ...
  const avatar = document.createElement('span'); avatar.className = 'avatar'; avatar.textContent = name.slice(0, 2).toUpperCase();
  avatar.setAttribute('aria-hidden', 'true');
  const info = document.createElement('span'); info.className = 'agent-info';
  const title = document.createElement('span'); title.className = 'agent-name'; title.textContent = name;
  const model = document.createElement('span'); model.className = 'agent-model'; model.textContent = `${agent.model} · ${agent.id.slice(-6)}`;
  info.append(title, model); button.append(avatar, info);
  button.addEventListener('click', () => requestSwitch(agent.path)); list.append(button);
  if (focusedAgent === agent.path) button.focus({ preventScroll: true });
 }
 if (!agents.length) { const note = document.createElement('p'); note.className = 'brand-sub'; note.textContent = 'No agents connected yet.'; list.append(note); }
}
