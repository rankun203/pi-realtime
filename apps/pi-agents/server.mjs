		...
		files
			...
			.map(async (name) => {
				try {
					...
					const response = await fetch(`http://127.0.0.1:${record.port}/pi-realtime/discovery`, {
					...
					return {
						id: record.id,
						port: record.port,
						project: String(info.project || "Pi"),
						sessions: info.sessions
							.filter((s) => sessionId.test(s.id))
							.map((s) => ({ id: s.id, model: String(s.model), mode: String(s.mode) })),
					};
				} catch {
					return;
				} // Dead/stale process or helper that no longer owns this port.
			}),
	);
...
export function createDashboard({
	...
	publicHost = process.env.PI_AGENTS_PUBLIC_HOST,
} = {}) {
	const server = createServer(
		(req, res) =>
			void handle(req, res).catch(() => reply(res, 502, { error: "Helper unavailable. Refresh the agent list." })),
	);
	async function handle(req, res) {
		...
		if (req.headers["sec-fetch-site"] === "cross-site")
			return reply(res, 403, { error: "Cross-origin request denied" });
		...
		res.setHeader(
			...
			"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src 'self' https:; media-src 'self' blob:; frame-src 'self'; frame-ancestors 'self'; object-src 'none'; base-uri 'none'",
		);
		...
		if (req.method === "GET" && url.pathname === "/api/agents") {
			...
			return reply(res, 200, {
				agents: helpers.flatMap((h) =>
					h.sessions.map((s) => ({
						...s,
						helperId: h.id,
						project: h.project,
						path: `/agents/${h.id}/pi-realtime/openai/${s.id}`,
					})),
				),
			});
		}
		const route =
			/^\/agents\/([a-f0-9-]{36})(\/pi-realtime\/(?:webrtc\/client\.js|openai\/[a-zA-Z0-9_-]{1,100}(?:\/(?:config|client-secret|event|outbox|messages|message|voice-connect|voice-heartbeat|voice-disconnect))?))$/.exec(
				url.pathname,
			);
		...
		for await (const chunk of req) {
			size += chunk.length;
			if (size > 256 * 1024) return reply(res, 413, { error: "Request too large" });
			chunks.push(chunk);
		}
		...
		const upstream = request(
			{ hostname: "127.0.0.1", port: helper.port, path: route[2] + url.search, method: req.method, headers },
			(response) => {
