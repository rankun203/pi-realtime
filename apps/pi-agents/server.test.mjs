...
test('dashboard discovers verified local helpers, proxies only registered sessions and rejects CSRF/rebinding', async () => {
 ...
 const helper = createServer((req, res) => {
  ...
  if (req.url === '/pi-realtime/discovery') return res.end(JSON.stringify({ id, project: '/workspace/demo', sessions: [{ id: 'session-a', model: 'mini', mode: 'agent' }] }));
 ...
 try {
  await writeFile(join(directory, `${id}.json`), JSON.stringify({ id, pid: process.pid, port: helperPort, host: 'ignored.invalid' }));
  ...
  const proxy = await fetch(`${base}${path}/messages`, { headers: { cookie: 'private-access-cookie' } }).then(r => r.json());
  ...
  assert.equal(post.status, 200); assert.equal(writes, 1);
  assert.equal((await fetch(`${base}${path}/message`, { method: 'POST', headers: { origin: 'https://evil.test' } })).status, 403);
  const rebindingStatus = await new Promise((resolve, reject) => {
   const req = request(`${base}/api/agents`, { headers: { host: 'evil.test' } }, res => { res.resume(); resolve(res.statusCode); });
  ...
  await writeFile(join(directory, `${id}.json`), JSON.stringify({ id: '22222222-2222-4222-8222-222222222222', pid: process.pid, port: helperPort }));
  assert.equal((await discoverHelpers(directory)).length, 0);
 } finally { await close(dashboard); await close(helper); await rm(directory, { recursive: true, force: true }); }
});
