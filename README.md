...
> Preview software. The companion transport was previously tested through Chromium's real WebRTC stack and Azure `gpt-realtime-2.1-mini`, including Pi delivery, result speech, device takeover, and provider hangup. The revised thin-relay policy has live Azure WebSocket text/synthetic-audio evidence but still needs live WebRTC/physical phone acceptance. Cloudflare deployment and the larger 2.1 model remain unvalidated.
...
`gpt-realtime-2.1-mini` now has verified Azure **Global Standard USD list estimates**, including historical observations previously marked unpriced. Per million tokens: text input/cached/output **$0.60/$0.06/$2.40**; audio **$10/$0.30/$20**; image input/cached **$0.80/$0.08**. [Azure meter evidence](.ai/validation/azure-mini-pricing.json). `gpt-realtime-2.1` is also verified: text input/cached/output **$4/$0.40/$24**, audio **$32/$0.40/$64**, image input/cached **$5/$0.50** per million tokens. [2.1 Azure meter evidence](.ai/validation/azure-realtime-2.1-pricing.json). Realtime returns usage, not dollar charges or a separate cache-write counter; estimates use returned modality/cache counts and exclude incomplete breakdowns. Data Zone, negotiated rates and actual invoices may differ. Other unknown models/custom deployment names remain explicitly unpriced rather than guessed.
...
Background provider and microphone/playback errors replace a single compact footer status instead of adding warnings to the Pi transcript. Full error messages are saved to the session JSONL trace; use `/realtime debug` to find its path, including after stopping voice. The footer reports when the log cannot be written and clears on extension shutdown/reload. Explicit command responses still appear normally.

Server-side failures are recorded in the session JSONL trace under the system temporary directory's `pi-realtime-traces/` folder. Requests without a live session (for example, an old browser tab polling a legacy URL) go to a `helper-errors-*.jsonl` file in that folder, never to Pi's terminal. `/realtime openai webrtc status` shows the helper error-log path once created. Stale session requests return HTTP 410 with instructions to open the current URL from Pi. If a session log fails, request errors fall back to the helper log; if both are unwritable, the HTTP error is still returned without printing to the terminal. The helper's `/config` response includes `debugTracePath`. Failed helper requests return an **Error ID** that matches a `request.failed` trace entry, with the route action, status, and safe diagnostics. Companion connection, cleanup, supervision, and control-channel failures also produce `voice.error` entries, including failures without an HTTP caller.
