...
assert.match(
	providerTypesSource,
	/sendToolResult\(result: VoiceToolResultRecord, policy\?: ToolResultResponsePolicy\)/,
);
...
assert.doesNotMatch(
	serviceSource,
	/from "\.\/providers\/openai"|from "@google\/genai"|openai\/realtime|createOpenAIRealtimeProvider|hasOpenAIRealtimeCredentials|createOpenAIWebRTCBridgeAdapter|createOpenAIWebRTCClientSecret/,
);
assert.doesNotMatch(
	serviceSource,
	/createMacOSFfmpegAudioCapture|AudioCaptureController|createFfplayAudioPlayback|AudioPlaybackController/,
);
assert.doesNotMatch(
	runtimeSource,
	/\.\/openai\/webrtc|createOpenAIWebRTCBridgeAdapter|createOpenAIWebRTCClientSecret|hasOpenAIWebRTCCredentials/,
);
assert.doesNotMatch(
	serviceSource,
	/setOpenAIWebRTCEnabled|isOpenAIWebRTCEnabled|startWebRTCHelper|stopWebRTCHelper|webRTCHelperStatus|sendInstructionFromTool|resolveCitation/,
);
...
assert.deepEqual(
	readdirSync(".pi/extensions/pi-realtime/providers").filter((name) => /^openai[-.]/.test(name)),
	[],
);
...
assert.equal(
	routeToolResult(pending, { voiceToolCallId: "call_openai", providerSessionId: "gemini_1" }),
