...
assert.match(openAIModelProfiles, /OPENAI_REALTIME_MODELS = \["gpt-realtime-mini", "gpt-realtime-2", "gpt-realtime-2\.1-mini", "gpt-realtime-2\.1"\] as const/);
...
assert.match(openAIModelProfiles, /model === "gpt-realtime-mini" \|\| model === "gpt-realtime-2\.1-mini"\) return WEAK_REALTIME_SPEECH_RENDERER_PROFILE/);
assert.match(openAIModelProfiles, /model === "gpt-realtime-2" \|\| model === "gpt-realtime-2\.1"\) return STRONG_REALTIME_SPEECH_RENDERER_PROFILE/);
...
assert.match(openAIRuntime, /availableModels\(\) \{ return \[\.\.\.new Set\(\[\.\.\.OPENAI_REALTIME_MODELS, openAIConnectionConfig\(\)\.model\]\)\]; \}/);
