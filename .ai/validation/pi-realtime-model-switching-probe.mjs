...
assert.match(
	types,
	/type ProviderPreferences = \{[\s\S]*autoMediaMode\?: ProviderMediaMode;[\s\S]*defaultModel\?: string;[\s\S]*\}/,
);
...
assert.match(
	openAIModelProfiles,
	/OPENAI_REALTIME_MODELS = \[\s*"gpt-realtime-mini",\s*"gpt-realtime-2",\s*"gpt-realtime-2\.1-mini",\s*"gpt-realtime-2\.1",?\s*\] as const/,
);
assert.match(
	openAIModelProfiles,
	/function openAIBehaviorProfileForModel\(model: string\): RealtimeBehaviorProfileFragment/,
);
assert.match(
	openAIModelProfiles,
	/import \{\s*STRONG_REALTIME_SPEECH_RENDERER_PROFILE,\s*WEAK_REALTIME_SPEECH_RENDERER_PROFILE,?\s*\} from "\.\.\/\.\.\/domain\/behavior-profiles"/,
);
assert.match(
	openAIModelProfiles,
	/model === "gpt-realtime-mini" \|\| model === "gpt-realtime-2\.1-mini"\)\s*return WEAK_REALTIME_SPEECH_RENDERER_PROFILE/,
);
assert.match(
	openAIModelProfiles,
	/model === "gpt-realtime-2" \|\| model === "gpt-realtime-2\.1"\)\s*return STRONG_REALTIME_SPEECH_RENDERER_PROFILE/,
);
...
assert.match(
	openAIRuntime,
	/availableModels\(\) \{\s*return \[\.\.\.new Set\(\[\.\.\.OPENAI_REALTIME_MODELS, openAIConnectionConfig\(\)\.model\]\)\];\s*\}/,
);
...
assert.match(
	behaviorProfiles,
	/WEAK_REALTIME_SPEECH_RENDERER_PROFILE[\s\S]*enabled: true[\s\S]*maxChars: 800[\s\S]*splitStrategy: "sentence"/,
);
assert.match(behaviorProfiles, /WEAK_REALTIME_SPEECH_RENDERER_PROFILE[\s\S]*envelope: "json_task"/);
assert.match(
	behaviorProfiles,
	/STRONG_REALTIME_SPEECH_RENDERER_PROFILE[\s\S]*systemPromptMode: "per_response_rendering"[\s\S]*envelope: "speech_source"[\s\S]*longTextThresholdChars: 300[\s\S]*longTextMode: "compact_summary"/,
);
...
assert.match(
	service,
	/defaultModelFor\(provider: ProviderKind\): string \{\s*return this\.providerPreference\(provider\)\.defaultModel \?\? this\.requireProviderRuntime\(provider\)\.defaultModel\(\);\s*\}/,
);
...
assert.match(service, /if \(!available\.includes\(model\)\)\s*throw new Error/);
assert.match(
	service,
	/providerPreferences: \{\s*\.\.\.this\.store\.state\(\)\.config\.providerPreferences,\s*\[provider\]: \{ \.\.\.this\.providerPreference\(provider\), defaultModel: model \},?\s*\}/,
);
