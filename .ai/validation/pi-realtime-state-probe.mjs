...
for (const kind of [
	...
	assert.match(typesSource, new RegExp(`kind: "${kind}"`));
...
function replay(events) {
	...
	for (const event of events.sort((a, b) => a.at - b.at || a.eventId.localeCompare(b.eventId))) {
		...
		if (event.kind === "context_packet_sent")
			state.context.set(`${event.providerSessionId}:${event.packet.channel}`, event.packet.revision);
...
const state = replay([
	...
	},
	{
		...
		at: 3,
		providerSessionId: "fake_a",
		packet: { packetId: "pkt_1", revision: 4, channel: "pi_state", priority: "normal", summary: "ok", createdAt: 3 },
	...
	},
	{
		...
		eventId: "evt-4",
		at: 4,
		result: { voiceToolCallId: "call_1", providerSessionId: "fake_a", status: "sent", resultText: "ok", at: 4 },
	},
	{
		version: 1,
		kind: "usage_observed",
