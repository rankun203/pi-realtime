...
const checks = [
	...
	},
	{
		...
		ok:
			design.includes("no final user transcript from any provider/media path may become a Pi backend instruction") &&
			design.includes("decideRealtimeInputGate"),
	},
	{
		name: "design converges shortcut, command, and model tool on one service operation",
		ok:
			design.includes("All control surfaces map to one service operation") &&
			design.includes("setListeningPaused") &&
			design.includes("ctrl+shift+m") &&
			design.includes("/realtime listening pause|resume|toggle|status") &&
			design.includes("realtime_set_listening"),
	},
	{
		name: "design persists pause through replay events and visible UI",
		ok:
			design.includes("listening_pause_changed") &&
			design.includes("Hydration from branch replay restores the pause flag") &&
			design.includes("ctx.ui.setStatus") &&
	...
	},
	{
		...
		ok:
			runtime.includes("ctx.ui.setStatus") &&
			runtime.includes("ctx.ui.setWidget") &&
			runtime.includes("registerRealtimeModelTools"),
	},
	{
		...
		ok:
			...
			service.includes("routeTranscriptToInstruction") &&
			service.includes("traceInstructionSubmission") &&
			service.indexOf("routeTranscriptToInstruction") < service.indexOf("traceInstructionSubmission"),
	},
	{
		name: "current commands and provider boundary expose the exact seams named in the design",
		ok:
			commands.includes("REALTIME_COMMANDS") &&
			commands.includes("realtimeCompletions") &&
			providerTypes.includes("RealtimeProviderAdapter") &&
			providerTypes.includes("setAudioOutputEnabled"),
	},
	{
		name: "WebRTC helper owns local media tracks and can implement track enabled toggling",
		ok:
			webrtcClient.includes("currentStream") &&
			webrtcClient.includes("getUserMedia") &&
			webrtcClient.includes("getAudioTracks"),
	},
];
