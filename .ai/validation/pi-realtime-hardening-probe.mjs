...
for (const event of ["session_start", "session_tree", "session_compact", "context", "session_shutdown"])
	assert.match(runtime, new RegExp(`pi\\.on\\("${event}"`));
...
function filter(messages, activeProviderSessionId, lastInstructionId) {
	return messages.filter((message) => {
		...
		if (message.details?.providerSessionId && message.details.providerSessionId !== activeProviderSessionId)
			return false;
...
const kept = filter(
	[
		...
		{
			role: "custom",
			customType: "pi-realtime.context",
			details: { providerSessionId: "active", instructionId: "i2" },
		},
