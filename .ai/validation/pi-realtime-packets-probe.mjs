...
for (const name of [
	"buildStatePacket",
	"buildCitationPacket",
	"buildToolSurfacePacket",
	"nextContextRevision",
	"buildCitationDeckFromBranch",
	"renderCitationDeck",
])
	assert.match(packetSource, new RegExp(`function ${name}`));
assert.match(promptSource, /name: "request"/);
assert.doesNotMatch(
	promptSource,
	/pi_state_snapshot|pi_send_instruction|pi_wait_for_update|pinotator_citation_resolve/,
);
...
const citationXml =
	'<pinotator_citation display_ref="[1]" alias="@p1" id="cit_abc" origin="transcript" source="assistant/message/raw_exact">Example &amp; citation</pinotator_citation>';
