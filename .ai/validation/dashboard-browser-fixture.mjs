...
for (const name of ['alpha', 'beta']) {
 ...
 helper.setDashboard({
  ...
  async sendMessage(text) {
   ...
   messages.push({ id: `a${messages.length}`, role: 'assistant', text: `Received: ${text}`, at: Date.now() + 1, source: 'Pi' });
  },
 });
 await helper.start();
 helper.registerSession({ provider: 'openai', providerSessionId: name, model: 'test-mini', interaction: { mode: 'agent', transcriptHandling: { response: 'native' } }, initialContext: { channel: 'state', revision: 1, summary: 'test', sections: [] }, createClientSecret: async () => ({ value: 'test-only', callsUrl: 'https://voice.invalid/calls' }) }, { onProviderEvent() {} });
 helpers.push(helper);
}
const server = createDashboard({ registryDir: join(process.env.PI_CODING_AGENT_DIR, 'pi-realtime', 'helpers') });
server.listen(0, '127.0.0.1', () => console.log(JSON.stringify({ port: server.address().port })));
process.on('SIGTERM', async () => { for (const helper of helpers) await helper.stop(); server.close(); server.closeAllConnections(); });
