...
import {
	...
	openAIWebSocketOptions,
} from "../../.pi/extensions/pi-realtime/providers/openai/connection";
import {
	OPENAI_REALTIME_MODELS,
	openAIBehaviorProfileForModel,
} from "../../.pi/extensions/pi-realtime/providers/openai/model-profiles";
...
function settings(openai: object) {
	writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ "pi-realtime": { openai } }));
}
	...
	try {
		...
		await t.test("global namespaced settings and isolated auth entry", () => {
			...
			writeFileSync(
				...
				JSON.stringify({
					openai: { type: "api_key", key: "unrelated-coding-key" },
					"pi-realtime:openai": { type: "api_key", key: "test-azure-key" },
				}),
			);
			const config = openAIConnectionConfig();
			assert.equal(config.apiKey, "test-azure-key");
			assert.equal(config.authMode, "api-key");
			assert.equal(config.baseURL, azure);
		...
		await t.test("validates config without echoing secrets", () => {
			...
			assert.throws(
				() => openAIConnectionConfig(),
				(error: Error) => !error.message.includes("do-not-echo") && error.message.includes("Invalid JSON"),
			);
			settings({ baseUrl: azure, authMode: "invalid" });
			assert.throws(() => openAIConnectionConfig(), /authMode/);
			for (const baseUrl of [
				...
				settings({ baseUrl });
				assert.throws(() => openAIConnectionConfig());
			}
			settings({ baseUrl: azure, model: 42 });
			assert.throws(() => openAIConnectionConfig(), /model must be/);
			settings({ baseUrl: azure, model: "gpt-realtime-2.1-mini" });
		});
		await t.test("HTTP Azure uses api-key only; OpenAI uses Bearer", async () => {
			for (const authMode of ["api-key", "bearer"]) {
				...
				const client = createOpenAIRealtimeClient(config).withOptions({
		...
		await t.test("WebRTC returns matching calls URL and only ephemeral credential", async () => {
			...
			const response = await createOpenAIWebRTCClientSecret({
				model: "gpt-realtime-2.1-mini",
				instructions: "Test",
				interaction: providerInteractionFor("eco"),
			});
			assert.equal(response.callsUrl, azure + "/realtime/calls");
			assert.equal(response.value, "ek_test");
			assert.ok(!JSON.stringify(response).includes("test-azure-key"));
		...
		await t.test("WebSocket strips SDK Bearer header for api-key authentication", () => {
			...
			assert.equal(removed, "Authorization");
		...
		await t.test("new models use existing speech profiles", () => {
			...
			assert.equal(
				openAIBehaviorProfileForModel("gpt-realtime-2.1-mini"),
				openAIBehaviorProfileForModel("gpt-realtime-mini"),
			);
		...
		await t.test("fixed loopback port configuration validates and can retry a failed bind", async () => {
			...
			for (const port of ["", "-1", "65536", "1.5", "abc"])
				assert.throws(() => loadRealtimeWebPort({ ...process.env, PI_REALTIME_WEB_PORT: port }), /integer/);
			...
			try {
				...
				occupied.close();
				await helper.stop();
				delete process.env.PI_REALTIME_WEB_PORT;
				settings({ baseUrl: azure, model: "gpt-realtime-2.1-mini" });
			}
		});
		await t.test("browser assets work outside extension checkout", async () => {
			...
			try {
				...
				helper.registerSession(
					{
						...
						model: "test",
						instructions: "Test",
						interaction: providerInteractionFor("agent"),
						toolSurface: { revision: 1, tools: [] },
						initialContext: {} as any,
						async createClientSecret() {
							issuedTokens++;
							return { value: "ek_test" };
						},
					},
					{ onProviderEvent() {} },
				);
				...
				const proxyStatus = (action: string, method: string) =>
				...
				assert.match(js, /fetch\(secret.callsUrl/);
				assert.ok(!js.includes("https://api.openai.com/v1/realtime/calls"));
			} finally {
				await helper.stop();
			}
		});
