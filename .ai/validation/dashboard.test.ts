...
test("chat projection excludes system, tool output and reasoning", () => {
 const entries = [
  ...
  { id: "b", type: "message", timestamp: "2026-01-02", message: { role: "assistant", content: [{ type: "thinking", thinking: "private reasoning" }, { type: "text", text: "Hi" }, { type: "toolCall", name: "bash" }] } },
  { type: "message", message: { role: "system", content: "private system" } },
  { type: "message", message: { role: "toolResult", content: "private output" } },
 ];
 assert.deepEqual(branchChatMessages(entries).map(m => m.text), ["Hello", "Hi"]);
});
...
test("helper chat works without voice, rejects cross-origin writes and deduplicates voice transcripts", async () => {
 ...
 const sent: string[] = [];
 helper.setDashboard!({ snapshot: () => ({ project: "demo", usage: "$0.01 est.", messages: [] }), async sendMessage(text) { sent.push(text); } });
 await helper.start();
 helper.registerSession({ provider: "openai", providerSessionId: "chat-test", model: "test", createClientSecret: async () => ({ value: "fake" }) } as any, { onProviderEvent() {} });
 const base = helper.urlFor("chat-test");
 try {
  assert.equal((await fetch(`${base}/messages`).then(r => r.json())).project, "demo");
  const post = (text: unknown, origin?: string) => fetch(`${base}/message`, { method: "POST", headers: { "content-type": "application/json", ...(origin ? { origin } : {}) }, body: JSON.stringify({ text }) });
  ...
  for (let i = 0; i < 2; i++) await fetch(`${base}/event`, { method: "POST", body: JSON.stringify({ type: "assistant_transcript", text: "Voice reply", final: true, providerEventId: "same" }) });
