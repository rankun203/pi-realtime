...
async function main() {
	...
	if (real)
		for (const key of ["OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_AUTH_MODE", "OPENAI_REALTIME_MODEL"])
			if (real[key]) process.env[key] = real[key];
	const { createAgentSession, DefaultResourceLoader, SessionManager, SettingsManager, AuthStorage, ModelRegistry } =
	...
	const { session } = await createAgentSession({
		...
		authStorage,
		modelRegistry,
		model: ai.getModel("openai", "gpt-4o"),
		thinkingLevel: "off",
		noTools: "all",
	...
	session.agent.streamFn = (model: any) => {
		...
		const out = {
			role: "assistant",
			content: [
				{ type: "text", text: "Fixture work completed. The diagnostic directory contains apple.txt and pear.txt." },
			],
			api: model.api,
	...
	helper.setDashboard!({
		pi: bridge,
		snapshot: () => ({ project: "fixture-workspace", messages: bridge.snapshot().messages, usage: "Fixture" }),
		sendMessage: async (text) => {
			await bridge.postMessage(text, "user");
		},
	});
	await helper.start();
	helper.registerSession(
		{
			...
			model: real?.OPENAI_REALTIME_MODEL || "test-mini",
			interaction: {
				mode: "agent",
				tools: [],
				toolChoice: "auto",
			...
			initialContext: { channel: "state", revision: 1, summary: "test", sections: [] } as any,
			createClientSecret: async () => {
				throw new Error("Browser must not request credentials");
			},
		},
		{ onProviderEvent() {} },
	);
			...
			if (req.method === "POST" && req.url === "/ask") {
				connections.at(-1)?.send?.({
					...
					item: {
						type: "message",
						role: "user",
						content: [{ type: "input_text", text: "Remind me of the two filenames we just found. Answer briefly." }],
					},
				});
				connections.at(-1)?.send?.({ type: "response.create", response: { output_modalities: ["audio"] } });
			}
			if (req.method === "POST" && req.url === "/pi")
				void session.prompt("Do the diagnostic work.").catch(console.error);
			res.setHeader("content-type", "application/json");
			res.end(
				JSON.stringify({
					completions,
					messages: session.messages,
					connections: connections.map(({ emit: _emit, send: _send, ...record }) => record),
	...
	console.log(
		JSON.stringify({ port: (dashboard.address() as any).port, control: (control.address() as any).port, live }),
	);
	process.on("SIGTERM", () => {
		void (async () => {
			await helper.stop();
...
function requireResolve() {
	return join(process.cwd(), "node_modules", "@earendil-works", "pi-coding-agent", "package.json");
}
