...
import {
	defaultVoiceToolSurface,
	voiceSystemPrompt,
	voiceSpeechRendererPrompt,
} from "../../.pi/extensions/pi-realtime/prompt";
...
test("agent audio turns do not depend on transcription", async (t) => {
	try {
		await t.test("native VAD creates responses, interrupts, and omits auxiliary transcription", () => {
			const audio = buildOpenAIRealtimeAudioConfig(
				openAIRealtimeAudioInput(providerInteractionFor("agent"), isolatedEnv),
			);
		...
		await t.test("eco still transcribes and never auto-responds", () => {
			const audio = buildOpenAIRealtimeAudioConfig(
				openAIRealtimeAudioInput(providerInteractionFor("eco"), isolatedEnv),
			);
		...
		await t.test("optional agent transcript is configurable but not a response gate", () => {
			writeFileSync(
				...
				JSON.stringify({ "pi-realtime": { openai: { transcriptionModel: "whisper-1" } } }),
			);
			const audio = buildOpenAIRealtimeAudioConfig(
				openAIRealtimeAudioInput(providerInteractionFor("agent"), isolatedEnv),
			);
			...
			assert.throws(
				() => openAIRealtimeAudioInput(providerInteractionFor("eco"), override),
				/requires a transcription model/,
			);
		});
		await t.test("raw optional transcript cannot create a duplicate native response; failures surface", () => {
			...
			adapter.sink = {
				onProviderEvent(event: any) {
					events.push(event);
				},
			};
			adapter.requestResponse = async () => {
				requests++;
			};
			adapter.handleServerEvent({
				type: "conversation.item.input_audio_transcription.completed",
				transcript: "Please ask Pi to list files.",
				event_id: "completed",
			});
			assert.equal(requests, 0);
			assert.ok(events.some((event) => event.type === "user_transcript"));
			adapter.handleServerEvent({
				type: "conversation.item.input_audio_transcription.failed",
				error: { code: "DeploymentNotFound", message: "Test failure" },
				event_id: "failed",
			});
		...
		await t.test("browser optional transcript cannot create duplicate responses; failures are visible", async () => {
			...
			vm.runInContext(
				`postEvent = async event => { posts.push(event); }; dc = { readyState: "open", send: data => sent.push(JSON.parse(data)) }; interactionConfig = { transcriptHandling: { response: "native" } };`,
				context,
			);
			await vm.runInContext(
				`handleInputAudioTranscription({ transcript: "Please ask Pi to list files.", event_id: "complete" })`,
				context,
			);
			assert.equal(sent.length, 0);
			assert.ok(posts.some((event) => event.type === "user_transcript"));
			await vm.runInContext(
				`handleRealtimeEvent({ type: "conversation.item.input_audio_transcription.failed", error: { code: "DeploymentNotFound", message: "Test failure" }, event_id: "failed" })`,
				context,
			);
